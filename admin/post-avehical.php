...
header('location:index.php');
    ...
    if(isset($_POST['submit'])) {
        ...
        if(strlen($vehicletitle) < 1) {
            $error[] = 'Vehicle title cannot be empty!';
        }
        ...
        if(strlen($vehicleoverview) < 1) {
            $error[] = 'Vehicle overview cannot be empty!';
        }

        if(strlen($priceperday) < 1) {
            $error[] = 'Price per day cannot be empty!';
        }

        if(!is_numeric($priceperday)) {
            $error[] = 'Price per day must contain only numbers!';
        }

        if(isset($fueltype) && $fueltype == '0') {
            $error[] = 'Please select fuel type!';
        }

        if(strlen($modelyear) < 1) {
            $error[] = 'Model year cannot be empty!';
        }

        if(!is_numeric($modelyear)) {
            $error[] = 'Model year must contain only numbers!';
        }

        if(strlen($seatingcapacity) < 1) {
            $error[] = 'Seating capacity cannot be empty!';
        }

        if(!is_numeric($seatingcapacity)) {
            $error[] = 'Seating capacity must contain only numbers!';
        }
...
<?php if($error){?><div class="errorWrap"><strong><?php if(count($error) == 1) { echo 'Error:'; } else { echo 'Errors: <br/>'; } ?></strong>
    <?php
    foreach ($error as $e) {
